...
function buildSimplePdf(lines) {
  ...
  lines.forEach(line => {
    ...
    const type = typeof line === 'object' ? line.type || 'normal' : 'normal';
    if (!text) { pageLines.push({ text: '', type }); return; }
    const width = type === 'title' ? 70 : type === 'table' ? 88 : 94;
    for (let i = 0; i < text.length; i += width) pageLines.push({ text: text.slice(i, i + width), type });
  });
  const pages = [];
  ...
  pages.forEach((page, pageIndex) => {
    const stream = [
      'q 0.05 0.21 0.12 rg 0 742 612 50 re f Q',
      'BT /F1 8 Tf 0.92 0.96 0.90 rg 50 766 Td (MECUZI FARM MANAGEMENT  |  INSTITUTIONAL REPORTING) Tj ET',
      ...page.flatMap((line, index) => {
        const y = 720 - index * 19;
        const safe = line.text.normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/[^\x20-\x7E]/g, '?').replace(/[()\\]/g, ch => `\\${ch}`);
        if (line.type === 'title') return [`q 0.84 0.91 0.81 rg 42 ${y - 8} 528 24 re f Q`, 'BT /F1 16 Tf 0.05 0.21 0.12 rg', `50 ${y} Td (${safe}) Tj ET`];
        if (line.type === 'section') return [`q 0.91 0.95 0.89 rg 42 ${y - 7} 528 19 re f Q`, 'BT /F1 10 Tf 0.05 0.21 0.12 rg', `50 ${y} Td (${safe}) Tj ET`];
        if (line.type === 'table') return [`q 0.95 0.97 0.94 rg 42 ${y - 7} 528 18 re f Q`, 'BT /F1 8 Tf 0.15 0.25 0.13 rg', `50 ${y} Td (${safe}) Tj ET`];
        return ['BT /F1 9 Tf 0.15 0.15 0.15 rg', `50 ${y} Td (${safe}) Tj ET`];
      }),
      `BT /F1 8 Tf 0.38 0.46 0.38 rg 50 28 Td (Confidential working paper  |  Page ${pageIndex + 1} of ${pages.length}) Tj ET`
    ].join('\n');
...
function exportPDFReportLegacy() {
  ...
  const styledLines = lines.map((line, index) => {
    ...
    if (index === 0) return { text, type: 'title' };
    if (/^[A-Z][A-Z &]+$/.test(text)) return { text, type: 'section' };
    if (text.startsWith('Period ')) return { text, type: 'table' };
    return { text, type: 'normal' };
  });
  downloadBlob(`investor_production_briefing_2026_${currentLocale}.pdf`, buildSimplePdf(styledLines), 'application/pdf');
...
function buildTraditionalChinesePdf(lines) {
  ...
  pages.forEach(page => {
    const stream = [
      'q 0.05 0.21 0.12 rg 0 742 612 50 re f Q',
      'BT /F1 11 Tf 1 1 1 rg 50 766 Td ' + utf16Hex('梅庫齊農場管理系統  |  理事會報告') + ' Tj ET',
      ...page.flatMap((line, index) => {
      ...
      'BT /F1 8 Tf 0.38 0.46 0.38 rg 50 28 Td ' + utf16Hex('機密工作文件  |  理事會簡報') + ' Tj ET'
    ].filter(Boolean).join('\n');
