...
> **⚠️ Chinese (zh-TW) requires a second font.** Poppins has no Traditional Chinese glyphs —
> see `I18N_SPEC.md` §7 for the required `Noto Sans TC` fallback stack and locale-aware
> application. Don't ship zh-TW pages using Poppins alone; text will silently render in the
...
- Asset provided: `design-assets/images/cashew-leaf-motif.png` (real cashew leaf, transparent
background — replaces the generic leaf from the original template; cashew is the actual crop
this software manages, so this is the correct motif to use going forward). The original
`leaf-motif-large.png` is kept only as a style reference and should not be shipped in the
product.

> Note: a cashew nut photo was also supplied for a produce/product image, but it had a visible
> stock-photo watermark and can't be used as-is. Swap in a properly licensed cashew nut photo
> before it goes into the live app (e.g. for the Production/harvest screens).
...
design-assets/
  images/
    cashew-leaf-motif.png    — real cashew leaf, transparent background (use this, not the generic leaf)
    leaf-motif-large.png     — original template leaf, kept as style reference only, do not ship
